...
class ClienteF {
    ...
    public function obtenerClientesCompletos($incluirInactivos = false) {
        try {
            ...
            error_log("Error en obtenerClientesCompletos: " . $e->getMessage());
            return [];
        }
    }
    
    public function obtenerClientesCompletosConPaginacion($offset = 0, $limit = 10, $incluirInactivos = false) {
        try {
            $query = "SELECT * FROM tcliente";
            ...
            $countQuery = "SELECT COUNT(*) as total FROM tcliente";
            if ($incluirInactivos) {
            ...
            error_log("Error en obtenerClientesCompletosConPaginacion: " . $e->getMessage());
            return ['clientes' => [], 'total' => 0];
        }
    }
    
    public function buscarClientes($nombre, $incluirInactivos = false) {
        try {
            $query = "SELECT * FROM tcliente
    ...
    public function buscarClientesConPaginacion($nombre, $offset = 0, $limit = 10, $incluirInactivos = false) {
        try {
            $query = "SELECT * FROM tcliente
                      WHERE nombre ILIKE $1";
            ...
            $countQuery = "SELECT COUNT(*) as total FROM tcliente WHERE nombre ILIKE $1";
            if ($incluirInactivos) {
            ...
            error_log("Error al buscar clientes con paginación: " . $e->getMessage());
            return ['clientes' => [], 'total' => 0];
        }
    }
    ...
    public function create($table, $data) {
        try {
            ...
            if ($table === 'tcliente') {
                return $this->createClienteEspecial($data);
            }
            ...
            $query = "INSERT INTO $table ($columns) VALUES ($values) RETURNING id_cliente";
    ...
    private function createClienteEspecial($data) {
        try {
            $query = "INSERT INTO tcliente 
                     (nombre, telefono, email, direccion, cantidad_producto, baja) 
            ...
            if (!$result) {
                throw new Exception("Error al crear cliente: " . pg_last_error($this->db));
            }
    ...
    public function clienteExiste($nombreCliente, $idCliente = null) {
        try {
            $query = "SELECT id_cliente FROM tcliente WHERE nombre = $1";
